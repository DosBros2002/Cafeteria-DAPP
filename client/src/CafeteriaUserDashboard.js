...
function CafeteriaUserDashboard() {
  ...
  const fetchMenuItems = async () => {
    try {
      // Connect to the MenuManagement contract (replace with your actual ABI and address)
      const contract = new web3.eth.Contract(Order_ABI, Order_ADDRESS);
      setOrderProcessingContract(contract);

      // Call the viewMenu function from the OrderProcessing contract
      const menuDetails = await contract.methods
        .getaddress()
        .call();
  ...
  const handlePlaceOrder = async () => {
    try {
      ...
      await orderProcessingContract.methods.placeOrder(itemIds, quantitiesArray).send({ from: (await web3.eth.getAccounts())[0] }); // Replace with the actual account
